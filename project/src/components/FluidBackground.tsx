...
export default function FluidBackground() {
  ...
  useEffect(() => {
    ...
    const simRes = isMobile ? 96 : 256;
    const dyeRes = isMobile ? 384 : 1024;
    ...
    function resize() {
      ...
      if (isMobile && lastW === w) return;
      lastW = w;
      const h = Math.max(window.innerHeight, window.screen ? window.screen.height : 0);
    ...
    function update() {
      if (isMobile) {
        ...
        if (now - lastFrameTime < 32) { requestAnimationFrame(update); return; }
        lastFrameTime = now;
      }
      if (!isVisible) return;
      const now = performance.now();
