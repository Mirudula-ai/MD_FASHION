...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-[100svh] flex items-center justify-center pt-20 pb-12 px-4">
      <div className="max-w-5xl mx-auto text-center relative z-10">
        ...
        <p className="font-heading text-3xl sm:text-5xl font-bold text-navy-700 mb-3 animate-fade-in">{business.name}</p>

        {/* One-line tagline, "Perfect Fit" in gold */}
        <h1
          className="font-heading font-bold text-navy-700 leading-tight mb-4 whitespace-nowrap"
          style={{ fontSize: lang === 'ta' ? 'clamp(0.68rem, 2.9vw, 2.2rem)' : 'clamp(0.82rem, 3.6vw, 2.6rem)' }}
        >
          {t('Custom Stitching · Perfect Fit · On-Time Delivery', 'தனிப்பயன் தையல் · சரியான அளவு · சரியான நேரத்தில் டெலிவரி')
            .split(' · ')
            .map((seg, i, arr) => (
              <span key={i}>
                <span className={`word-anim inline-block ${i === 1 ? 'gold-text' : ''}`} style={{ animationDelay: `${i * 0.2}s` }}>{seg}</span>
                {i < arr.length - 1 && <span className="gold-text mx-[0.35em]">·</span>}
              </span>
            ))}
        </h1>

        <p className="text-navy-500 text-base sm:text-lg mb-4 animate-fade-up" style={{ animationDelay: '0.8s' }}>
          {t('Two Boutiques in Chennai', 'சென்னையில் இரண்டு கிளைகள்')}
        </p>
